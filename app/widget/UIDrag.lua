...
function UIDragItem:ctor(box,obj,z,tag,ap,bCenter)
    --拖拽物
    self.dragObj = nil
    --拖拽物以其中心点设定坐标
    self.bCenter = true
    ...
    --拖拽物在盒子内坐标(0~1),默认为盒子中间[self.bCenter==false启用]
     self.dragAP = ap or cc.p(0.5, 0.5)
    --拖拽盒
    self.dragBox = box
    --拖拽对象
    self.dragObj = nil
...
--[[--
把一个对象设置在盒子里面，对象会以居中或自定义的方式显示在盒子内。
对象的显示层级是通过设置z来实现的
]]
function UIDragItem:setDragObj(obj)
    if obj then
        ...
        local w = W(self.dragBox);
        local h = H(self.dragBox);
        ...
        if self.bCenter then
            ...
            if self.dragBox:isIgnoreAnchorPointForPosition() then
                ...
                x = x - w*AX(self.dragBox) + w*self.dragAP.x
                y = y - h*AY(self.dragBox) + h*self.dragAP.y
            end
...
function UIDrag:ctor(list,isModel)
    ...
    --忽略不可见节点
    self._ignoreInvisible = true
    --触摸的空位置
    self._TouchEmptyItem = nil
    ...
    --触摸空位置
    self._onTouchEmptyPosEvent = function(currentItem, point) end
...
function UIDrag:setOnTouchEmptyPosEvent(fun)
    self._onTouchEmptyPosEvent = fun
end
...
function UIDrag:addDragItem(item,drag,z,group,ap,bCenter)
    if item.__cname == "UIDragItem" then
        self._dragItems[#self._dragItems+1] = item
    else
        local item = UIDragItem.new(item,drag,z,group,ap,bCenter)
        self._dragItems[#self._dragItems+1] = item
    end
...
function UIDrag:handler_ScrollView(scroll,point)
    if scroll:getScrollNode():MyHitTest(point) then
...
function UIDrag:onTouchBegan(point)
    --遮盖物
    if self._ignoreInvisible then
        for i = 1,#(self._hinderList) do
            if self._hinderList[i] and self._hinderList[i]:isVisible() and self._hinderList[i]:MyHitTest(point) then
                return false
            end
        end
    else
        ...
        repeat
            ...
            if box:MyHitTest(point) then
                if item.dragObj then
                    ...
                    local p = self._currentDragItem.dragObj:convertToWorldSpaceAR(cc.p(0,0))
                    --定位到drag坐标
...
function UIDrag:onTouchMoved(point)
    ...
    if  self._isOnBean then
        ...
        if self._isModel then
            local _newModel = copySpineModel(self._currentDragObj)
            self._currentDragObj:removeFromParent()
            self._currentDragObj = _newModel
            self:addChild(_newModel)
        else
...
function UIDrag:onTouchEnded(point)
    if self._TouchEmptyItem then
        self._onTouchEmptyPosEvent(self._TouchEmptyItem, point)
        self._TouchEmptyItem = nil
        return
    end
    ...
    for i = 1 , #(self._dragItems) do
        ...
        if self._currentDragItem.dragBox:MyHitTest(point) then
...
--                self._currentDragItem = nil
            break
        --拖拽离开盒子
        elseif  box:MyHitTest(point) then
            isDownbefore = false
            --拖拽成功之前
            ...
            if not target.dragObj then
                if self._isModel then
                    ...
                    --###ios新加
                    self._currentDragObj:removeFromParent()
                    self._currentDragItem.dragObj = nil
                    --当前框设置空
                    -- self._currentDragItem:setDragObj(nil)
                    --目标框存放
                    -- self._currentDragObj:setOpacity(255)
            ...
            break  --放下位置即使有多个重叠的box，也只与首个做处理
        end
    end
    ...
    if not result then
        ...
        -- if self._currentDragObj.makeType==ModelMakeType.kMake_Coco then
        --     self._currentDragObj:retain()
        --     self._currentDragObj:removeFromParent()
        --     self._currentDragItem:setDragObj(nil)
        --     self._currentDragObj:setOpacity(255)
